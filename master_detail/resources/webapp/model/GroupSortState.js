sap.ui.define([
	...
	return BaseObject.extend("lines.model.GroupSortState", {
		 ...
		 * @alias lines.model.GroupSortState
		 */
		...
		/**
		 * Sorts by FILENAME, or by PAGENUMBER
		 *
		...
		/**
		 * Groups by PAGENUMBER, or resets the grouping for the key "None"
		 *
		 * @param {string} sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "PAGENUMBER") {
				...
				this._oViewModel.setProperty("/sortBy", "PAGENUMBER");

				aSorters.push(
					new Sorter("PAGENUMBER", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "FILENAME");
			}
