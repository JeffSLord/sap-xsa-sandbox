				...
				actions : {
							...
							success : function (aEntityData) {
								this.waitFor({
									...
									success : function (oList) {
										var sCurrentId,
											aItemsNotInTheList = aEntityData.filter(function (oObject) {
												return !oList.getItems().some(function (oListItem) {
													return oListItem.getBindingContext().getProperty("FILENAME") === oObject.FILENAME;
												});
											});

										if (!aItemsNotInTheList.length) {
											// Not enough items all of them are displayed so we take the last one
											sCurrentId = aEntityData[aEntityData.length - 1].FILENAME;
										} else {
											sCurrentId = aItemsNotInTheList[0].FILENAME;
										}
										...
										oCurrentItem.bindingPath = "/" + oList.getModel().createKey("LINES", {
											FILENAME : sCurrentId
										});
					...
					iRememberTheListItem : function (oListItem) {
						...
						this.getContext().currentItem = {
							...
							id: oBindingContext.getProperty("FILENAME"),
							title: oBindingContext.getProperty("FILENAME")
						};
					}
				},
				...
				assertions : {
					...
					theListHeaderDisplaysZeroHits : function () {
						return this.waitFor({
							viewName : sViewName,
							id : "page",
							matchers : new PropertyStrictEquals({name : "title", value : "LINES (0)"}),
							success : function () {
								Opa5.assert.ok(true, "The list header displays 'LINES (0)'");
							},
					...
					theListShouldBeSortedAscendingOnUnitNumber : function () {
						return this.theListShouldBeSortedAscendingOnField("PAGENUMBER");
					},

					theListShouldBeSortedAscendingOnName : function () {
						return this.theListShouldBeSortedAscendingOnField("FILENAME");
					},
					...
					theListShouldHaveAllEntries : function () {
						...
						// retrieve all LINES to be able to check for the total amount
						...
						return this.waitFor({
							...
							success : function (oList) {
								...
								this.waitFor({
									id : "page",
									viewName : sViewName,
									matchers : new PropertyStrictEquals({name : "title", value : "LINES (" + iExpectedLength + ")"}),
									success : function () {
